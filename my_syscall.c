...
asmlinkage long sys_my_syscall(int a, int b, char *c)
{
	...
	// set task to init_task
	task = &init_task;
	task = next_task(task);

	// cycle through the requested number of tasks
	for (i = 0; i < a; i++) {
		task = next_task(task); 
	}
	
	// set first buffer entry to null if tasks have cycled back to init_task
	if (a > 1 && (int)task->pid == 0) {
		kbuf[0] = '\0';
	}
	else {
